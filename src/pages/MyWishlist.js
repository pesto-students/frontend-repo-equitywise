...
const MyWishlist = () => {
  ...
  useEffect(() => {
    ...
    const fetchStockData = async (symbol) => {
      try {
        const response = await fetch(`https://finnhub.io/api/v1/quote?symbol=${symbol}&token=${apiKeyFinnhub}`);
        ...
        if (data) {
          ...
          setStocks(prevStocks => prevStocks.map(stock => 
            ...
            ? { 
                ...
                [stockAttributes.OVERALL_GAIN]: ((data.c - stock[stockAttributes.AVG_COST]) * stock[stockAttributes.NO_OF_SHARES]).toFixed(2),
    ...
    }, 300000); // Fetch data every minute
  ...
  const handleStockNameInputChange = async (e) => {
    const userInput = e.target.value;
    console.log('User Input:', userInput);
    setNewStock({ ...newStock, [stockAttributes.STOCK_NAME]: userInput });
    if (userInput.length >= 2) {
      try {
        ...
        // Filter suggestions to match the first letter typed by user and security type "Common Stock"
  ...
  //    try {
  //      console.log('Making API Call');
  //      const response = await axios.get(`https://finnhub.io/api/v1/search?q=${userInput}&token=${apiKeyFinnhub}`);
  //      console.log('Symbol Suggestions Response:', response.data);
  //      setSymbolSuggestions(response.data.result || []);
  //    } catch (error) {
  //      console.error('Error fetching symbol suggestions:', error);
  //    }
  //  } else {
