...
const Login = () => {
  ...
  const { setUsername } = useUser(); // Destructure setUsername correctly from useUser

  const onSuccess = (response) => {
    console.log('Login Success:', response);
    login(response.data);
    setUsername(email); // Update username state with email
...
//      console.log('API response:', response);
//      if (response.status === 201) {
//        //Need to change to 200 after deploying 
//        onSuccess(response);
//      } else {
//        setErrorMessage('Invalid email or password');
//      }
